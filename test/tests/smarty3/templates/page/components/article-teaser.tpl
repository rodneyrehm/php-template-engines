<article {if !empty($id)}id="{$id|escape}"{/if} class="post-{$post.id|escape} post type-post status-{$post.status|escape} format-{$post.format|escape} {if $post.hentry}hentry{/if} {foreach $post.categories as $category}category-{$category.key|escape}{/foreach} {foreach $post.tags as $tag}tag-{$tag.key|escape}{/foreach} post">
    ...
    <ul class="postmetadata clearfix">
        ...
        <li class="tags">{foreach $post.tags as $tag}<a href="{$tag.url|escape}">{$tag.name|escape}</a>{if !$tag@last}, {/if}{/foreach}</li>
